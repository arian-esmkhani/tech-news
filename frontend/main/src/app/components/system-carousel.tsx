...
export function SystemCarousel() {
    ...
    const { operatingSystemSlider } = useSlider()
    const router = useRouter();

    useEffect(() => {
        const fetchNewest = async () => {
            const data = await operatingSystemSlider();
    ...
    return(
        ...
        h-68 sm:h-89 rounded-2xl pt-4">
            <span className="ml-36 sm:ml-9 text-lg sm:text-xl opacity-65">Operating System Articles</span>
